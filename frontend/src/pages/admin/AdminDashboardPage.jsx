...
const AdminDashboardPage = () => {
  ...
  // Active Tab
  const [activeTab, setActiveTab] = useState('all'); // 'all', 'users', 'analytics'
  ...
  const fetchStats = async () => {
    ...
    try {
      ...
      setStatsError(err.message || 'Failed to load platform analytics');
    } finally {
  ...
  const statCards = [
    { label: 'Total Platform Users', value: stats?.totalUsers || 0, icon: Users, color: 'bg-blue-600' },
    { label: 'Active Users', value: stats?.activeUsers || 0, icon: UserCheck, color: 'bg-emerald-600' },
    { label: 'Administrator Accounts', value: stats?.adminUsers || 0, icon: Shield, color: 'bg-purple-600' },
    { label: 'Standard Users', value: stats?.normalUsers || 0, icon: User, color: 'bg-slate-700' },
    { label: 'Active Subscriptions', value: stats?.activeSubscriptions || 0, icon: Mail, color: 'bg-indigo-600' },
    { label: 'Total Saved Cities', value: stats?.totalSavedCities || 0, icon: Building2, color: 'bg-teal-600' },
    { label: 'Active Weather Alerts', value: stats?.activeAlerts || 0, icon: Bell, color: 'bg-rose-600' },
    { label: 'Weather History Logs', value: stats?.totalWeatherHistoryRecords || 0, icon: History, color: 'bg-amber-600' },
  ];
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Unified administration console for platform analytics, user accounts, and security controls
            </p>
          </div>
        </div>
        ...
        <button
          onClick={reloadAll}
          className="inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-white hover:bg-slate-50 text-slate-700 font-bold rounded-2xl text-xs border border-slate-200 shadow-sm transition"
        >
          <RefreshCw className="w-4 h-4" /> Refresh All Data
        </button>
      </div>

      {/* SECTION 1: SYSTEM ANALYTICS METRICS */}
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-extrabold uppercase tracking-wider text-slate-400 flex items-center gap-2 font-heading">
            <Activity className="w-4 h-4 text-blue-500" /> Platform Overview & Analytics
          </h2>
        </div>
        ...
        {statsLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {statCards.map((c, i) => {
              const Icon = c.icon;
              return (
                <div key={i} className="rounded-3xl bg-white border border-slate-200/80 p-5 shadow-sm card-hover flex items-center gap-4">
                  <div className={`p-3.5 text-white rounded-2xl shadow-md ${c.color}`}>
                    <Icon className="w-6 h-6" />
                  </div>
                  <div>
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{c.label}</span>
                    <p className="text-2xl font-extrabold text-slate-900 font-heading mt-0.5">{c.value}</p>
                  </div>
                </div>
              );
            })}
      ...
      {/* SECTION 2: USER MANAGEMENT CONSOLE */}
      <div className="space-y-4 pt-2">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <h2 className="text-sm font-extrabold uppercase tracking-wider text-slate-400 flex items-center gap-2 font-heading">
            <Users className="w-4 h-4 text-purple-500" /> Registered User Accounts
          </h2>
        ...
        {usersLoading && <LoadingSkeleton type="table" count={5} />}
